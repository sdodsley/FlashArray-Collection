...
def _get_interface(module, array):
    ...
    if "ct" in module.params["name"]:
        res = array.get_network_interfaces(names=[module.params[module.params["name"]]])
        if res.status_code != 200:
            return None
        return list(res.items)[0]
    return None
...
def create_vif(module, array, interface, subnet):
    ...
    if not module.check_mode:
        ...
        if not module.params["enabled"]:
            res = array.patch_network_interface(
...
def delete_vif(module, array, subnet):
    ...
    if not module.check_mode:
        ...
        res = array.delete_network_interface(names=[vif_name])
        if res.status_code != 200:
